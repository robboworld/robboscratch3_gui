...
class ColorCorrectorTableComponent extends Component {
      ...
      setInterval(() => {
        ...
        if (this.props.color_corrector_table.isShowing){
          ...
          if (sensors_data[0] != -1){
            ...
            color_box.style.minHeight = '40px';
            color_box.style.border = '2px solid';
            color_box.style.backgroundImage = "url('./static/robbo_assets/icon_close_window.png')";
  ...
  calculatePercents(color, id) {
    ...
    if(typeof(rgb_arr)!='undefined'){
      switch(color){
        case "R": return Math.round( rgb_arr[0]*100/sum ) +  "%";
        case "G": return Math.round( rgb_arr[1]*100/sum ) +  "%";
        case "B": return Math.round( rgb_arr[2]*100/sum ) +  "%";
      }
